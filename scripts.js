window.addEventListener('load', () => {
  ...
  function diplayStatusText(context) {
    ...
    if (gameOver) {
      ...
      context.fillText(`Your Score: ${score} `, canvas.width / 2 + 2, 251)
      context.fillText(
        `Press Enter or Swipe down to restart  `,
        canvas.width / 2 + 2,
        301
  ...
  function toggleFullScreen() {
    ...
    if (!document.fullscreenElement) {
      canvas.requestFullscreen().catch((err) => {
        alert(`Error, cannot enable fullscreen ${err}`)
      })
    } else {
  ...
  fullScreenMode.addEventListener('click', toggleFullScreen)
  const input = new InputHandler()
  const player = new Player(canvas.width, canvas.height)
  const background = new Backgroung(canvas.width, canvas.height)
